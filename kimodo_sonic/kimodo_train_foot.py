#!/usr/bin/env python3
"""Kimodo distance-reference finetuning launcher with foot tracking rewards.
...
Compared with ``kimodo_train.py``, this variant also includes both feet in the
position and linear-velocity tracking rewards.
...
DEFAULT_PROJECT_NAME = "kimodo_position_foot_finetune"
...
KIMODO_FOOT_REWARD_BODIES = [
    "left_ankle_roll_link",
    "right_ankle_roll_link",
]
KIMODO_POS_VEL_REWARD_BODIES = KIMODO_UPPER_BODY_REWARD_BODIES + KIMODO_FOOT_REWARD_BODIES
...
def add_common_overrides(overrides: list[str], args: argparse.Namespace, motion_file: Path) -> None:
    overrides.extend(
        [
            ...
            f"{hydra_list(KIMODO_POS_VEL_REWARD_BODIES)}",
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--experiment_name", type=str, default=f"kimodo_pos_foot_track_{timestamp}")
    ...
    parser.add_argument(
        "--subprocess-log",
        type=str,
        default=str(REPO_ROOT / "kimodo_sonic" / "kimodo_train_foot_subprocess.log"),
    )
    parser.add_argument(
        "--override",
        action="append",
